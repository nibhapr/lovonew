...
export const Header = memo(
	({
		...
		return (
			<header
				className={`${props.className ? props.className : ""}${scrollPos.y > 5 ? " sticky-header" : ""}${scrollPos.directionDown === false ? " header-appear" : ""}${
			...
			>
				<ul className="navbar-nav">
					{props.data.map((item, i) => {
						return (
							<li
								className={`nav-item${item.dropdown || item.megamenu ? ` dropdown` : ""}${isMenuActive === i ? " open" : ""}`}
								key={i}
							>
								{item.link ? (
								...
								{item.megamenu && (
									<div className="flex megamenu" ref={megamenu_ref}>
										{item.megamenu.map((item, i) => {
											return (
												<ul
													className={`${item.dropdown.filter((item) => item.img).length > 0 ? "!pr-[30px] img-wrapper inline-block last:!pr-[0px]" : "inline-block"}`}
													key={i}
												>
													{item.title && (
...
export const SearchBar = memo((props) => {
	...
	return (
		...
		>
			...
			>
				<i
					className={`feather-search px-0 inline-block${props.className ? ` ${props.className}` : ""}`}
...
export const CollapsibleMenu = () => {
	...
	return (
		<Accordion
			ref={collapsibleMenu}
			className={`collapsible-menu${theme ? ` ${theme}` : ""}${
				className ? ` ${className}` : ""
			}`}
		>
			{HeaderData &&
				HeaderData.map((item, i) => {
					return (
						<Accordion.Item key={i} eventKey={i}>
							<Accordion.Header>
								{item.link ? (
									<a aria-label="link" className="menu-link" to={item.link}>
										{" "}
										{item.title}{" "}
									</a>
								) : (
