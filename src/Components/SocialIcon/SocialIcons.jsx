...
const SocialIcons = ({
	...
	return (
		<ul
			className={`social-icon flex-wrap gap-y-5 p-0 ${theme} ${size} ${iconColor} ${className}`}
		>
			{data.map((item, i) => {
				return theme !== "social-icon-style-1" ? (
					<motion.li
						key={i}
					...
					>
						...
						>
							{item.name && (
								<span className="flex brand-label">
									{item.name ? item.name : "icon"}
								</span>
							)}
